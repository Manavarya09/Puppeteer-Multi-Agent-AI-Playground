...
function Hero() {
  return (
    <section className="relative overflow-hidden border-b hairline">
      ...
      <div className="absolute inset-0 noise" style={{ ['--grain-opacity' as any]: 0.06 }} />
        ...
        <div className="grid grid-cols-12 gap-8 mt-12">
          <div className="col-span-12 md:col-span-5 md:col-start-2">
            <p className="text-[19px] leading-[1.55] text-bone-300/90 text-balance">
              Puppeteer is a research-grade platform that coordinates a team of specialist
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/playground" className="px-5 py-3 bg-signal-amber text-ink-950 font-mono text-[12px] tracking-[0.22em] uppercase hover:bg-bone-100 transition-colors">Run a task →</Link>
              <Link to="/research" className="px-5 py-3 border hairline-strong font-mono text-[12px] tracking-[0.22em] uppercase hover:bg-bone-100 hover:text-ink-950 transition-colors">Read the paper</Link>
            </div>
          </div>
...
function SimulatedTerminal() {
  const lines = [
    ...
    { t: '', cls: '' },
    { t: 'task_complete  tokens=8,420  cost=$0.013  topology=cyclic', cls: 'text-signal-sage' },
  ]
  ...
  return (
    <div className="border hairline-strong bg-ink-900/70 corner relative">
      ...
      <div className="border-b hairline px-3 py-2 flex items-center justify-between">
        <span className="eyebrow">terminal · puppeteer.ai/run</span>
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-24 grid grid-cols-12 gap-8">
        <div className="col-span-12 md:col-span-4">
          ...
          <p className="text-[15px] text-bone-300/85 mt-4 leading-relaxed">
            ...
            gVisor-sandboxed containers with no network egress except to allow-listed APIs.
          </p>
        </div>
        <div className="col-span-12 md:col-span-8 grid grid-cols-2 gap-px bg-bone-100/10">
          {[
            ['Frontend', 'React 18 · TypeScript · Vite', 'Playground UI, live graph, account.'],
            ['API gateway', 'FastAPI · Uvicorn', 'Auth, rate limiting, SSE streaming.'],
            ['Orchestrator', 'PyTorch · 4-layer Transformer', 'Pre-trained policy, online RL.'],
            ['Agent pool', 'Docker · gVisor', 'Each agent isolated. 512MB RAM. 30s timeout.'],
            ['LLM router', 'Multi-provider', 'OpenAI · Anthropic · Groq · Ollama.'],
            ['Task queue', 'Redis Streams', 'Async dispatch · pub-sub state bus.'],
            ['Database', 'PostgreSQL 16 · Multi-AZ', '99.999% durability. Read replicas.'],
            ['Observability', 'Datadog APM + Logs + Metrics', 'Per-agent token attribution.'],
          ].map(([title, tech, desc]) => (
...
function PricingPreview() {
  const tiers = [
    { name: 'Free', price: '$0', cap: '5 tasks / day', features: ['Mimas subspace', 'Final-snapshot graph', 'Basic transparency', 'Community forum'] },
    { name: 'Pro', price: '$29', cap: '200 tasks / day', features: ['Mimas + Titan', 'Live graph + replay', 'Full transparency', '90-day history', 'API access'], featured: true },
    { name: 'Teams', price: '$99', cap: '2,000 / day · org', features: ['Custom agents', 'SSO + roles', '1-year history', 'Slack support', 'Webhooks'] },
    { name: 'Research', price: '$0', cap: '100 tasks / day', features: ['Full interaction logs', 'Topology replay', '2-year history', 'Reproducible benchmarks'] },
  ]
  return (
    <section className="border-b hairline">
      <div className="max-w-[1400px] mx-auto px-6 py-24">
        <div className="grid grid-cols-12 mb-12">
          <div className="col-span-12 md:col-span-6">
            ...
            <h2 className="font-display text-[64px] leading-[0.95] mt-3 text-bone-100">Honest, <span className="serif-italic text-signal-amber">tier'd</span>.</h2>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-bone-100/10">
          {tiers.map(t => (
            <div key={t.name} className={`p-6 ${t.featured ? 'bg-bone-100 text-ink-950' : 'bg-ink-900'}`}>
              ...
              <ul className={`mt-6 space-y-2 text-[13px] ${t.featured ? 'text-ink-700' : 'text-bone-300/85'}`}>
                {t.features.map(f => <li key={f} className="flex gap-2"><span className={`mt-[8px] w-2 h-px ${t.featured ? 'bg-ink-950' : 'bg-signal-amber'}`} />{f}</li>)}
              </ul>
            </div>
          ))}
        </div>
  ...
  return (
    <section className="relative">
      <div className="max-w-[1400px] mx-auto px-6 py-32 text-center">
        <span className="eyebrow">§ 07 · what now</span>
        ...
        <Link to="/playground" className="inline-block mt-12 px-8 py-4 bg-signal-amber text-ink-950 font-mono text-[13px] tracking-[0.22em] uppercase hover:bg-bone-100 transition-colors">Open the playground →</Link>
        <p className="mt-6 eyebrow">no credit card · 5 free tasks per day · model: mimas</p>
      </div>
    </section>
  )
}
