...
interface Node {
  id: string; x: number; y: number; vx: number; vy: number; fired: boolean
}
...
export function Graph({ invocations, edges, activeId, cycles, height = 360 }: Props) {
  ...
  useEffect(() => {
    ...
    for (const id of firedIds) {
      if (!map.has(id)) {
        ...
        map.set(id, {
          id, x: size.w / 2 + Math.cos(angle) * r, y: size.h / 2 + Math.sin(angle) * r,
  ...
  const cycleSet = useMemo(() => {
    ...
    for (const c of cycles) for (let i = 0; i < c.length - 1; i++) set.add(`${c[i]}->${c[i + 1]}`)
    return set
  }, [cycles])
  ...
  return (
    <svg ref={ref} viewBox={`0 0 ${size.w} ${size.h}`} style={{ width: '100%', height }} preserveAspectRatio="xMidYMid meet">
      <defs>
      ...
      {edges.map(e => {
        ...
        return (
          <path key={`${e.from}-${e.to}`} d={`M ${from.x} ${from.y} Q ${mx} ${my} ${to.x} ${to.y}`}
            fill="none" stroke={stroke} strokeWidth={w}
            markerEnd={isCycle ? 'url(#arrow-amber)' : 'url(#arrow)'} opacity={0.85} />
        )
      })}
      ...
      {firedIds.map(id => {
        ...
        return (
          <g key={id} transform={`translate(${n.x} ${n.y})`}>
            ...
            <text x={0} y={4} textAnchor="middle" fontFamily='"JetBrains Mono", monospace' fontSize={10} fill={text} letterSpacing={0.6}>{spec.short}</text>
            <text x={0} y={34} textAnchor="middle" fontFamily='"Geist", sans-serif' fontSize={9} fill="rgba(246,241,230,0.55)" letterSpacing={0.4}>{spec.name.replace('Agent','')}</text>
          </g>
        )
      })}
    </svg>
