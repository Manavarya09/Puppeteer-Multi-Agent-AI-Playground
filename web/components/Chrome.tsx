...
function NavItem({ to, label }: { to: string; label: string }) {
  ...
  return (
    <Link href={to} className={`relative pb-1 transition-colors ${isActive ? 'text-bone-100' : 'hover:text-bone-100'}`}>
      {label}
  ...
  return (
    <footer className="border-t hairline mt-24">
      <div className="max-w-[1400px] mx-auto px-6 py-12 grid grid-cols-2 md:grid-cols-5 gap-8 text-[13px] text-bone-300/80">
        ...
        <Col title="Product" links={[['/playground', 'Playground'], ['/agents', 'Agent library'], ['/docs', 'API & SDKs']]} />
  ...
  return (
    <div>
      ...
      <ul className="space-y-2">
        {links.map(([to, label]) => (
          <li key={label}>
            <Link href={to} className="hover:text-bone-100 transition-colors">{label}</Link>
          </li>
        ))}
      </ul>
    </div>
